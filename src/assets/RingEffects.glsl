...
vec3 ramp(float t) {
	return t <= 2. ? vec3( /* 1. - t * 1.4, .2, 1.05 */0 ) / t : vec3( .3 * (1. - t) * 2., .2, 1.05 ) / t;
}
...
void main() {
    ...
    float t = iTime;
    vec2 uv = (gl_FragCoord.xy-iResolution.xy*0.5)/iResolution.y * 2. + .5;
    float ff = uv.y;
    vec2 uv2 = uv;
    ...
    vec4 output4 = vec4(c1 + c2, 1.0);
    output4 = (output4.r > .07 && output4.g > .07 && output4.b > .07) ? output4 : output4*vec4(vec3(1), 0);
